...
| Vanguard | 1.2275 | 0.7255 | 計入敏捷閃避 +150% 被動加成 (等效傷害乘以 0.5910) |
...
- Sentinel (防禦力 +150%)：
  - 依據 [[combat-formulas]] 的公式：Mitigation = 0.5 + 0.435 × LOG10(Defense / Strength)
  - 假設基礎屬性比為 1:1 (減傷 50%)，防禦力變為 2.5 倍後，減傷率提升至 67.31%。
  - 折算因子 = (1 - 0.6731) / (1 - 0.50) = 0.6538
- Vanguard (敏捷度 +150%)：
  - 依據 Wiki 實測數據的對稱插值（比值變為 1/2.5 = 0.4 倍，對應之優勢 2.5 倍命中率約為 70.45%）：敏捷變為 2.5 倍後，命中率實際上降低至約 29.55%（而非簡化公式算出的 38.94%）。
  - 折算因子 = 29.55% / 50.00% = 0.5910
...
3. Delta 在純減傷期望值上表現較差，但其價值體現在負面狀態抗性上。Vanguard 在計入敏捷被動提升後的防護等效係數可達 0.7255，生存能力其實直逼 Sentinel (0.6651)，且具備極高敏捷帶來的閃避/隱匿優勢。
